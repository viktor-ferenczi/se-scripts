...
namespace ProjectorInventory
{
    public class Program: SpaceEngineersProgram
    {
        ...
        private IEnumerable<StringBuilder> FormatSummary(int panelRowCount)
        {
            ...
            var sortedSummary = missing.ToList().OrderBy(pair => componentTranslation[pair.Key]);
            foreach (KeyValuePair<string, double> item in sortedSummary)
            {
                var formattedAmount = string.Format("{0:n0}", Math.Round(item.Value));
                var line = formattedAmount.PadLeft(maxWidth) + " " + componentTranslation[item.Key];
