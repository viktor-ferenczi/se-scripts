...
static StringBuilder bsodBuilder = new StringBuilder();
public static void PrintBsod(Program program, IMyTextSurface surface, string scriptName, string version, float fontSize, Exception e)
{
...
bsodBuilder.Clear();
program.Echo(exceptionLines.Length.ToString());
...
{
if (capacity < 1) throw new Exception($"Capacity of CircularBuffer ({capacity}) can not be less than 1");
Capacity = capacity;
...
Program _program;
public MissileStatusScreenHandler(Program program, ref Program.StatusScreenColors colors)
{
_program = program;
...
public void UpdateColors(ref Program.StatusScreenColors colors)
{
...
if (ratio >= 0.5)
{
return new Color(color1.R + (byte)(scale * (float)color2.R), color1.G + (byte)(scale * (float)color2.G), color1.B + (byte)(scale * (float)color2.B));
}
return new Color(color2.R + (byte)(2 * ratio * (float)color1.R), color2.G + (byte)(2 * ratio * (float)color1.G), color2.B + (byte)(2 * ratio * (float)color1.B));
}
public void ComputeScreenParams(Program.GuidanceMode mode, Program.GuidanceMode allowedModes, float lockStrength, string statusText, Color statusColor, double range, bool inGravity, bool stealth, bool spiral, bool topdown, bool precise, bool autofire, bool fireEnabled)
{
_spriteContainers.Clear();
var guidanceMode = mode;
var allowedModesEnum = allowedModes;
bool showTopdownAndAimMode = true;
bool anyGuidanceAllowed = allowedModesEnum != Program.GuidanceMode.None;
...
if (guidanceMode == Program.GuidanceMode.BeamRiding)
{
...
{
if (guidanceMode == Program.GuidanceMode.Camera)
{
modeSelectSize = MODE_CAMERA_SELECT_SIZE;
modeSelectPos = MODE_CAMERA_SELECT_POS;
}
else if (guidanceMode == Program.GuidanceMode.Turret)
{
...
container = new MySpriteContainer(MODE_CAMERA_TEXT, FONT, SECONDARY_TEXT_SIZE, MODE_CAMERA_POS, (allowedModesEnum & Program.GuidanceMode.Camera) != 0 ? _guidanceAllowedColor : _guidanceDisallowedColor, TextAlignment.CENTER);
_spriteContainers.Add(container);
container = new MySpriteContainer(MODE_TURRET_TEXT, FONT, SECONDARY_TEXT_SIZE, MODE_TURRET_POS, (allowedModesEnum & Program.GuidanceMode.Turret) != 0 ? _guidanceAllowedColor : _guidanceDisallowedColor, TextAlignment.CENTER);
_spriteContainers.Add(container);
container = new MySpriteContainer(MODE_BEAMRIDE_TEXT, FONT, SECONDARY_TEXT_SIZE, MODE_BEAMRIDE_POS, (allowedModesEnum & Program.GuidanceMode.BeamRiding) != 0 ? _guidanceAllowedColor : _guidanceDisallowedColor, TextAlignment.CENTER);
_spriteContainers.Add(container);
// Range
...
else return new MySprite(SpriteType.TEXT, _text, center + _positionFromCenter * scale, null, _color, _font, rotation: _rotationOrScale * scale, alignment: _textAlign);
}
}
...
{
if (defaultChar.HasValue) return defaultChar.Value;
...
{
public QueuedAction(Action action, double runInterval) : base(action, 1.0 / runInterval, removeAfterRun: true, timeOffset: 0)
{
}
}
...
{
get { return OffsetTargeting ? OffsetTargetPosition : TargetCenter; }
}
...
if (!_fudgeVectorSwitch) return Vector3D.Zero;
...
if (!Vector3D.IsUnit(ref perpVector2)) perpVector2.Normalize();
...
*/
if (reference == null) reference = GetControlledCamera(cameraList);
if (reference == null) reference = referenceBlock;
if (reference == null) reference = GetControlledShipController(shipControllers);
if (reference != null)
{
...
if (availableScanRange > scanRange && _timeSinceLastScan >= AutoScanInterval)
{
if (Status == TargetingStatus.Locked || _manualLockOverride) _targetInfo = thisCamera.Raycast(adjustedTargetPos);
else if (!Vector3D.IsZero(testDirection)) _targetInfo = thisCamera.Raycast(cameraMatrix.Translation + testDirection * MaxRange);
else _targetInfo = thisCamera.Raycast(MaxRange);
...
{
if (_manualLockOverride && TargetId != _targetInfo.EntityId && TargetId != 0) return;
...
if (TestCameraAngles(block, vectorIsPosition ? testVector - block.GetPosition() : testVector)) availableCameras.Add(block);
}
}
...
{
if (shipControllers.Count == 0) return null;
...
if (mainController != null) return mainController;
...
{
get { return TicksSinceLastRun * Scheduler.TickDurationSeconds; }
}
public long TicksSinceLastRun { get; private set; } = 0;
public double RunInterval
{
get { return RunIntervalTicks * Scheduler.TickDurationSeconds; }
set { RunIntervalTicks = (long)Math.Round(value * Scheduler.TicksPerSecond); }
}
public long RunIntervalTicks
{
get { return _runIntervalTicks; }
set
{
if (value == _runIntervalTicks) return;
...
set
{
if (value == _runFrequency) return;
if (value == 0) RunIntervalTicks = long.MaxValue;
else RunIntervalTicks = (long)Math.Round(Scheduler.TicksPerSecond / value);
}
}
...
if (!_inUpdate) _scheduledActions.Add(scheduledAction);
else _actionsToAdd.Add(scheduledAction);
}
...
double _maxSpeedToAllowFire = 85; // m/s
/*
...
INI_MAX_SPEED_TO_ALLOW_FIRE = "Maximum speed to allow fire",
// Sound config
...
Dictionary<string, GuidanceMode> _guidanceModeDict = new Dictionary<string, GuidanceMode>() { { "CAMERA", GuidanceMode.Camera }, { "TURRET", GuidanceMode.Turret }, { "BEAMRIDE", GuidanceMode.BeamRiding }, };
...
private Vector3D pbVelocity = Vector3D.Zero;
...
var duration = Runtime.TimeSinceLastRun.TotalSeconds;
var pos =  pbs[0].GetPosition();
if (firstUpdate)
{
firstUpdate = false;
} else
{
pbVelocity = duration > 0 ? (pos - lastPosition) / duration : Vector3D.Zero;
}
lastPosition = pos;
_runtimeTracker.AddRuntime();
if ((updateType & (UpdateType.Terminal | UpdateType.Trigger | UpdateType.Script)) != 0) ParseArguments(arg);
...
var lastRuntime = RuntimeToRealTime * Math.Max(duration, 0);
if (_autoFire) _timeSinceAutoFire += lastRuntime;
try
{
_scheduler.Update();
...
Bsod.PrintBsod(this, Me.GetSurface(0), scriptName, VERSION, 1f, e);
foreach (var surface in _textSurfaces)
{
Bsod.PrintBsod(this, surface, scriptName, VERSION, 1f, e);
}
_showBSOD = true;
}
...
SendMissileParameterMessage(killNow, _stealth, _spiral, _topdown, _usePreciseAiming, _retask, broadcastKey);
if (_retask) _retask = false;
if (killNow) _hasKilled = true;
if (_broadcastRangeOverride) _broadcastRangeOverride = false;
}
...
// Antenna range
shouldBroadcast = true;
if (_stealthySemiActiveAntenna) antennaRange = Vector3D.Distance(base.Me.CubeGrid.WorldAABB.Center, _raycastHoming.TargetPosition) - _raycastHoming.TargetSize;
else antennaRange = _activeAntennaRange;
// Broadcast
long broadcastKey = GetBroadcastKey();
...
// Play sound
if (!_raycastHoming.MissedLastScan) PlayLockOnSound(_soundBlocks);
else if (_raycastHoming.MissedLastScan) PlayScanMissedSound(_soundBlocks);
// Status
...
{
if (_autoFireRemote) RequestRemoteMissileFire();
else FireNextMissile(1);
...
// Antenna range
shouldBroadcast = true;
if (_stealthySemiActiveAntenna) antennaRange = Vector3D.Distance(Me.CubeGrid.WorldAABB.Center, _targetInfo.Position) - 10.0;
else antennaRange = _activeAntennaRange;
// Broadcast
long broadcastKey = GetBroadcastKey();
SendMissileHomingMessage(
...
if (!_remoteFireResponses.Contains(response)) _remoteFireResponses.Add(response);
}
}
}
}
...
{
if (!(obj is RemoteFireResponse)) return false;
...
{
if (i + 1 > _remoteFireRequests) break;
...
GridTerminalSystem.GetBlocksOfType((List<IMyMechanicalConnectionBlock>)null, b =>
{
double rad = b.CubeGrid.WorldVolume.Radius;
IMyCubeGrid grid = b.CubeGrid;
if (b.IsAttached)
{
double radT = b.TopGrid.WorldVolume.Radius;
if (radT > rad)
{
rad = radT;
grid = b.TopGrid;
...
TypeMask = LargeGrid | SmallGrid | Other
}
void NetworkTargets()
{
...
if ((_designationMode == GuidanceMode.Camera && _raycastHoming.Status == RaycastHoming.TargetingStatus.Locked) || (_designationMode == GuidanceMode.Turret && _turretLocked))
{
...
get
{
if (pbVelocity.Length() > _maxSpeedToAllowFire)
return false;
...
case "mode_beamride":
case "mode_optical":
if (_opticalAllowed) _designationMode = GuidanceMode.BeamRiding;
break;
case "mode_camera":
...
break;
case "mode_turret":
if (_turretAllowed) _designationMode = GuidanceMode.Turret;
break;
case "lock_on":
if (_semiActiveAllowed)
{
...
{
if (_allowedGuidanceModes.Count == 0) return;
...
if (_designationMode == GuidanceMode.Camera) _raycastHoming.ClearLock();
}
bool GrabBlocks()
{
...
else _allowedGuidanceModes.Add(GuidanceMode.Camera);
...
else _allowedGuidanceModes.Add(GuidanceMode.Turret);
...
if (_designationMode == GuidanceMode.None) _designationMode = _allowedGuidanceModes[0];
_setupStringbuilder.AppendLine("\n>>> SETUP SUCCESSFUL <<<");
...
_maxSpeedToAllowFire = _setupIni.Get(INI_SECTION_GENERAL, INI_MAX_SPEED_TO_ALLOW_FIRE).ToDouble(_maxSpeedToAllowFire);
...
_setupIni.Set(INI_SECTION_GENERAL, INI_MAX_SPEED_TO_ALLOW_FIRE, _maxSpeedToAllowFire);
...
if (block.CustomName.IndexOf(_referenceNameTag, StringComparison.OrdinalIgnoreCase) >= 0) _reference = block;
...
if (display) textSurfaces.Add(surfaceProvider.GetSurface(i));
_textSurfaceIni.Set(INI_SECTION_TEXT_SURF, iniKey, display);
}
string output = _textSurfaceIni.ToString();
if (!string.Equals(output, block.CustomData)) block.CustomData = output;
}
...
if (!missileNumberDict.TryGetValue(missileNumber, out group)) return false;
...
if (reference == null) reference = GetControlledCamera(_cameraList);
if (reference == null) reference = GetControlledShipController(_shipControllers);
if (reference == null)
{
...
if (thisRange > _maxTurretRange) _maxTurretRange = thisRange;
}
}
...
var payload = new MyTuple<Matrix3x3, Matrix3x3, float, long, long> { Item1 = matrix1, Item2 = matrix2, Item3 = (float)timeSinceLastLock, Item4 = targetId, Item5 = keycode, };
...
{
var payload = new MyTuple<Vector3, Vector3, Vector3, Vector3, long> { Item1 = (Vector3)forward, Item2 = (Vector3)left, Item3 = (Vector3)up, Item4 = (Vector3)shooterPosition, Item5 = keycode };
...
var payload = new MyTuple<byte, long> { Item1 = packedBools, Item2 = keycode };
...
{
long broadcastKey = -1;
if (_broadcastList.Count > 0) broadcastKey = _broadcastList[0].EntityId;
return broadcastKey;
}
...
foreach (var block in turrets)
{
if (block.IsUnderControl) return block;
}
return null;
}
IMyShipController GetControlledShipController(List<IMyShipController> shipControllers)
...
if (mainControlled != null) return mainControlled;
if (controlled != null) return controlled;
