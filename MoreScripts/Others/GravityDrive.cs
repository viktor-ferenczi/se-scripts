...
{
    partial class Program: SpaceEngineersProgram
    {
        const string nameTag = "Scouter GDrive";
...
//-----------------------------------------------
        ...
        void Main(string argument)
        {
            ...
            if (isSetup && (timeCurrentCycle >= timeMaxCycle))
            {
                try
                {
                    ...
                    Echo(
                        $"Non-GDrive Gravity Generators: {gravityGeneratorsExclude.Count + gravityGeneratorsSphereExclude.Count}");
                    ...
                    var desiredDirection = referenceMatrix.Backward * inputVec.Z + referenceMatrix.Right * inputVec.X +
                                           referenceMatrix.Up * inputVec.Y; // World relative input vector.
        ...
        bool GrabBlocks()
        {
            ...
            GridTerminalSystem.GetBlocksOfType(gravityGeneratorsExclude,
                block => block.CubeGrid == referenceList[0].CubeGrid);
            ...
            GridTerminalSystem.GetBlocksOfType(gravityGeneratorsSphereExclude,
                block => block.CubeGrid == referenceList[0].CubeGrid);
        ...
        void ApplyThrust(Vector3D travelVec, double speed, Vector3D desiredDirectionVec, bool dampenersOn)
        {
            ...
            {
                var thrustDirection =
                    thisGravityGenerator.WorldMatrix.Up; //gets the direction that the thruster flame fires
                float scale =
                    -(float) thrustDirection.Dot(
                        desiredDirectionVec); //projection of the thruster's direction onto the desired direction

                if (scale >= maxThrustDotProduct)
                {
                    scale /=
                        (float) fullBurnDotProduct; //scales it so that the thruster output ramps down after the fullBurnToleranceAngle is exceeded
                    ...
                    if (velocityInThrustDirection < 1)
                        ...
                        targetOverride = velocityInThrustDirection * Math.Abs(velocityInThrustDirection) *
                                         dampenerScalingFactor;
                ...
                thrustDirection =
                    thisGravityGenerator.WorldMatrix.Down; //gets the direction that the thruster flame fires
                scale = -(float) thrustDirection.Dot(
                    desiredDirectionVec); //projection of the thrust's direction onto the desired direction
                ...
                if (dampenersOn && (!isAntiJerkOn || (isAntiJerkOn && (speed > dampenerAntiJerk))))
                {
                    thrustDirection =
                        thisGravityGenerator.WorldMatrix.Up; //gets the direction that the thruster flame fires
                    scale = -(float) thrustDirection
                        .Dot(travelVec); //projection of the thrust's direction onto the desired direction

                    if ((scale >= maxThrustDotProduct) &&
                        (thrustDirection.Dot(desiredDirectionVec) <= minDampeningDotProduct))
                    {
                        var velocityInThrustDirection = thrustDirection.Dot(travelVec) * speed;
                        double targetOverride = 0;
                        ...
                        if (velocityInThrustDirection < 1)
                            ...
                            targetOverride = velocityInThrustDirection * velocityInThrustDirection *
                                             dampenerScalingFactor;
            ...
            else
            {
                ...
                for (int i = 0; i < gravityGeneratorsExclude.Count && gravityGeneratorsExcludeStandby; ++i)
                {
                    gravityGeneratorsExclude[i]
                        .ApplyAction((gravityGeneratorsExcludeEnable[i]) ? "OnOff_On" : "OnOff_Off");
                }

                // Sphere Non-GDrive Gravity Generator
                for (int i = 0; i < gravityGeneratorsSphereExclude.Count && gravityGeneratorsExcludeStandby; ++i)
                {
                    gravityGeneratorsSphereExclude[i]
                        .ApplyAction((gravityGeneratorsSphereExcludeEnable[i]) ? "OnOff_On" : "OnOff_Off");
                }
